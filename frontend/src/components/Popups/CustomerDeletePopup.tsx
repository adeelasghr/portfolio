...
import { X, Mail, Phone, User, Code, Check } from "lucide-react";
import { useEffect, useState } from "react";
import { deleteCustomer, getCustomersBrief } from "../../api/customerApi";
...
const CustomerDeletePopup: React.FC<CustomerDetailsPopupProps> = ({
  customerId,
  onClose,
}) => {
  const [custData, setCustData] = useState<CustomersBrief | null>(null);
 ...
 const handleDelete = () => {
  ...
  useEffect(() => {
    const fetchCustomerCode = async () => {
      try {
        const data = (await getCustomersBrief(customerId)) as CustomersBrief[];
        setCustData(data[0]);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="p-8">
            <h3 className="card-header text-xl mb-6">Delete Customer</h3>
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <span>{custData?.code}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <User className="text-blue-500" size={20} />
                      <span>{custData?.fullName}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <Phone className="text-blue-500" size={20} />
                      <span>{custData?.phone}</span>
                    </div>
      ...
      {showSuccessModal && (
        ...
        >The customer has been successfully deleted</MessageBox>
      )}
    </>
  );
};
